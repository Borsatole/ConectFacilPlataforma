...
if ($verificaIp['bloqueado']) {
    echo json_encode([
        'erroMessage' => 'Você está fazendo muitas tentativas. Aguarde ' . $verificaIp['tempoRestante'] . ' segundos para tentar novamente.'
    ]);
    exit;
}
...
if (!$dadosRecebidos) {
    echo json_encode(['erroMessage' => 'Nenhum dado recebido']);
    exit;
}
...
if (!$dadosLogin) {
    RegistrarTentativa();
    echo json_encode(['erroMessage' => 'Login ou senha incorretos']);
    exit;
}
